...
def analisar_fluxo_m1(simbolo, df_m1, direcao_macro):
    ...
    if corpo_vela > 150 and vol_1 < media_vol and abs(saldo_delta) < (media_vol * 0.1):
        return False, f"BLOQUEIO: Vela oca (Delta: {saldo_delta})."
    ...
    if direcao_macro == "COMPRA" and muralha_venda is not None:
        ...
        if 0 < distancia_resistencia < 50: # Se a muralha está a menos de 50 pontos acima
            return False, f"BLOQUEIO DOM: Muralha vendedora detectada em {muralha_venda} ({dom['muralha_venda_vol']} contratos)."
    ...
    if direcao_macro == "VENDA" and muralha_compra is not None:
        ...
        if 0 < distancia_suporte < 50: # Se a muralha está a menos de 50 pontos abaixo
            return False, f"BLOQUEIO DOM: Muralha compradora detectada em {muralha_compra} ({dom['muralha_compra_vol']} contratos)."
    ...
    if direcao_macro == "COMPRA":
        ...
        if (tamanho_pavio_inf / tamanho_vela) >= 0.60 and vol_1 > media_vol:
            ...
            if saldo_delta < 0:
                return True, f"FURA-FILA: Absorção Perfeita. Bateram {abs(saldo_delta)}, lote passivo segurou.{msg_extra}"
            else:
                return True, f"FURA-FILA: Rejeição compradora (Delta: +{saldo_delta}).{msg_extra}"
                
        if exaustao_confirmada and preco_fechamento >= vela_atual['open']:
            return True, "FURA-FILA: Exaustão confirmada. Secou o lote vendedor."
        ...
        if (tamanho_pavio_sup / tamanho_vela) >= 0.60 and vol_1 > media_vol:
            ...
            if saldo_delta > 0:
                return True, f"FURA-FILA: Absorção Perfeita. Tomaram {saldo_delta}, lote passivo travou.{msg_extra}"
            else:
                return True, f"FURA-FILA: Rejeição vendedora (Delta: {saldo_delta}).{msg_extra}"
                
        if exaustao_confirmada and preco_fechamento <= vela_atual['open']:
            return True, "FURA-FILA: Exaustão confirmada. Secou o lote comprador."
...
def confirmar_sequencial(simbolo, tipo_posicao_aberta):
    """
    ...
    if tipo_posicao_aberta == "COMPRA":
        if saldo_delta >= DELTA_GATILHO:
            return True, f"🌊 Sequencial Confirmado! Robôs tomando a mercado (Delta: +{saldo_delta})."
            
    elif tipo_posicao_aberta == "VENDA":
        if saldo_delta <= -DELTA_GATILHO:
            return True, f"🌊 Sequencial Confirmado! Robôs batendo a mercado (Delta: {saldo_delta})."

    return False, f"Aguardando tração do fluxo (Delta atual: {saldo_delta})"